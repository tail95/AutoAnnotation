...
def addBndBox(xmin, ymin, xmax, ymax, name, difficult, boxlist):
...
def visualize_boxes_and_labels_on_image_array(
    ...
    folderName, file = os.path.split(file_name)
    et = Element('annotation')
    #
    folder = SubElement(et, 'folder')
    folder.text = folderName


    #
    filename = SubElement(et, 'filename')
    filename.text = file
    #

    localImgPath = SubElement(et, 'path')
    localImgPath.text = file_name
    #
    source = SubElement(et, 'source')
    database = SubElement(source, 'database')
    database.text = "Unkown"
    #
    size_part = SubElement(et, 'size')
    ...
    #
    segmented = SubElement(et, 'segmented')
    segmented.text = '0'
    ...
    for box, color in box_to_color_map.items():
        ...
        # print(class_name)
        addBndBox(left,bottom,right,top, class_name, 0, boxlist)
        print()
        print()

        ############

    appendObjects(et,boxlist)
    output_filename = str(folderName + "\\" + str(file.split(".")[0]))
    print(output_filename+XML_EXT)
    out_file = codecs.open(output_filename+XML_EXT, 'w' ,encoding='utf-8')
    prettifyResult = prettify(et)
